...
// Interface disesuaikan agar kompatibel penuh dengan tipe data Prisma (null-safe)
...
export default function PortfolioSection({
  ...
  return (
    <section className="bg-white text-slate-900 font-sans py-20">
      <div className="max-w-7xl mx-auto px-6">
        
        {/* ── 1. HEADER SECTION (2 COLUMNS LAYOUT) ── */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-end mb-12">
          
          {/* Kolom Kiri: Subtitle + Title + Pill Button */}
          <motion.div
          ...
          {/* Kolom Kanan: Deskripsi Paragraf + Arrow Navigation */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: "-50px" }}
            transition={{ duration: 0.6, delay: 0.1 }}
            className="lg:col-span-5 flex flex-col justify-between space-y-8 lg:pl-6"
          >
            {description && (
        ...
        {projects && projects.length > 0 ? (
            ...
            {projects.map((project, index) => (
              ...
              >
                ...
                >
                  ...
                  {/* View Badge Center */}
                  <div className="absolute inset-0 m-auto w-12 h-12 rounded-full bg-white/20 backdrop-blur-md border border-white/40 text-white text-xs font-medium flex items-center justify-center opacity-0 scale-75 group-hover:opacity-100 group-hover:scale-100 transition-all duration-300">
                    Lihat
                  </div>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-5 space-y-1 text-white">
                    ...
                    {project.location && (
                      <p className="text-slate-300 text-xs tracking-wide truncate">
                        {project.location}
                      </p>
                    )}
                  </div>
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="w-full py-4 px-6 border border-blue-500/30 bg-blue-50/50 rounded-md text-blue-600 text-xs md:text-sm font-normal"
          >
