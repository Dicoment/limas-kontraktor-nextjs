...
export default async function PublicBlogDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="min-h-screen max-w-4xl mx-auto py-12 px-8">
      ...
      <p className="text-slate-600">Categories: {(post.blogPostCategories || []).map((c: any) => c.category.name).join(", ") || "—"}</p>
      <p className="text-slate-600">Tags: {(post.blogPostTags || []).map((t: any) => t.tag.name).join(", ") || "—"}</p>
