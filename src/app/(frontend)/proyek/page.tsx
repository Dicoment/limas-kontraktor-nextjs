...
export default async function ProyekPage({ searchParams }: Props) {
  ...
  return (
    <main className="min-h-screen bg-[#F8F9FA] font-sans antialiased">
      {/* ── CSS KEYFRAMES INLINE FOR HIGH PERFORMANCE ANIMATION ── */}
      <style dangerouslySetInnerHTML={{__html: `
        @keyframes limasFadeInUp {
          from {
            opacity: 0;
        ...
        .animate-fade-in-up {
          animation: limasFadeInUp 0.7s cubic-bezier(0.16, 1, 0.3, 1) forwards;
        }
      ...
      >
        ...
        {/* Kontainer Utama Jadi Center-Aligned + Animasi */}
        <div className="max-w-7xl mx-auto text-center flex flex-col items-center w-full relative z-10 animate-fade-in-up stagger-1">
          ...
          <p className="text-white/80 text-base md:text-lg max-w-xl leading-relaxed mb-8">
            Kumpulan proyek konstruksi dan renovasi terbaik kami yang telah berhasil diselesaikan di Bekasi, Jakarta, and Seluruh Indonesia.
          </p>
      ...
      <section className="max-w-7xl mx-auto px-6 py-10 animate-fade-in-up stagger-2">
      ...
      {/* ── GRID LAYOUT ── */}
      <section className="max-w-7xl mx-auto px-6 pb-24 animate-fade-in-up stagger-3">
        {allProjects.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {allProjects.map((project: { id: string; slug: string; title: string; description: string | null; coverImage: string | null; client: string | null; location: string | null; status: string; categoryProjects: { catEntry: { id: string; name: string } }[]; createdAt: Date }, index: number) => (
              <Link
                key={project.id}
